...
class DataUploader:
    def __init__(self, api_url,
        ...
        Args:
            api_url (str or None): The API endpoint URL for data uploads. If None, caching is disabled.
        ...
        self.caching_enabled = api_url is not None
        if not self.caching_enabled and debug:
            print("[INFO] Caching disabled because api_url is None.")
        ...
        # RLock because we may need to acquire it multiple times in nested calls
        ...
        self.source = source  # Default source for metadata

        # Only initialize cache-related functionality if caching is enabled
        if self.caching_enabled:
            try:
                os.makedirs(self.cache_files_dir, exist_ok=True)
                if self.debug:
                    print(
                        f"[INFO] Ensured cache files directory exists: {self.cache_files_dir}")
            except OSError as e:
                print(
                    f"[ERROR] Could not create cache files directory {self.cache_files_dir}: {e}. File caching will be impaired.")
            ...
            # Set cache_files_dir to None to disable file operations
            self.cache_files_dir = None
            if self.debug:
                print("[DEBUG] Cache initialization skipped (caching disabled).")
    ...
    def _get_mac_address(self):
        ...
        try:
            # Added common enpXsX names
            for iface_name in ['eth0', 'wlan0', 'en0', 'enp0s3', 'enp1s0']:
            ...
            if self.debug:
                print(f"[DEBUG] Error reading MAC from /sys: {e}")

        if self.debug:
            print("[INFO] MAC address: Falling back to uuid.getnode().")
        try:
            ...
            if self.debug:
                print(
                    f"[WARNING] uuid.getnode() returned {mac_int} (formatted: {formatted_mac}), which might be a locally administered or non-ideal MAC.")
            return formatted_mac
        except Exception as e:
            if self.debug:
                print(f"[WARNING] Error using uuid.getnode() for MAC: {e}")
        ...
        with self.cache_lock:
            if os.path.exists(self.cache_file_path):
                try:
                    ...
                    if self.debug:
                        print(
                            f"[INFO] Loaded {len(self.failed_sends_cache)} items from cache: {self.cache_file_path}")
                except json.JSONDecodeError:
                    if self.debug:
                        print(
                            f"[ERROR] Could not decode cache file: {self.cache_file_path}. Starting fresh.")
                ...
                if self.debug:
                    print(
                        f"[DEBUG] Cache file {self.cache_file_path} not found. Starting fresh.")
                self.failed_sends_cache = []
        ...
        try:
            ...
            if cache_dir and not os.path.exists(cache_dir):
                try:
                    ...
                    if self.debug:
                        print(
                            f"[DEBUG] Could not create cache directory {cache_dir}: {e_mkdir}")
            ...
            if self.debug:
                print(
                    f"[DEBUG] Cache saved successfully to {self.cache_file_path}. Size: {len(self.failed_sends_cache)}")
            return True
        except Exception as e:
            ...
            if self.debug:
                print(
                    f"[ERROR] Could not save cache to {self.cache_file_path}: {e}")
    ...
    def _remove_files_for_cached_item(self, item_to_clean):
        ...
        if item_to_clean and item_to_clean.get("cached_files") and self.cache_files_dir:
            if self.debug:
                print(
                    f"[DEBUG] Cleaning up disk files for item: {item_to_clean.get('original_identifier')}")
            for _form_field, file_info in item_to_clean["cached_files"].items():
                ...
                if cached_file_path_to_delete and os.path.exists(cached_file_path_to_delete):
                    try:
                        ...
                        if self.debug:
                            print(
                                f"  - Deleted cached file from disk: {cached_file_path_to_delete}")
                    except Exception as e:
                        if self.debug:
                            print(
                                f"  - [ERROR] Failed to delete cached file {cached_file_path_to_delete} from disk: {e}")
    ...
    def _add_to_cache(self, data_payload, url_to_use, files_tuple_dict, original_identifier, is_heartbeat_type, messages_list_for_warning):
        ...
        if not self.caching_enabled:
            return  # Caching is disabled, do not proceed
        ...
        with self.cache_lock:  # Acquire lock for modifying cache and saving
            ...
            if self._save_cache():  # _save_cache does not acquire lock
                ...
                if cached_files_info:
                    log_msg += f" {len(cached_files_info)} file(s) associated."
                ...
                for path in temp_files_written_paths:
                    try:
                        ...
                        if self.debug:
                            print(
                                f"[ERROR] Failed to clean up temp file {path} after cache save failure: {e_del}")
    ...
    def _remove_from_cache_by_item(self, item_to_remove):
        with self.cache_lock:  # Acquire lock
            self._remove_files_for_cached_item(
                item_to_remove)  # This helper does not lock
            try:
                ...
                if self.debug:
                    print(
                        f"[INFO] Removed item ({item_to_remove.get('original_identifier')}) from cache. Cache size: {len(self.failed_sends_cache)}")
            except ValueError:
                if self.debug:
                    print(
                        f"[WARNING] Item ({item_to_remove.get('original_identifier')}) not found in cache for removal (possibly already removed).")
    ...
    def _send_data_thread(self, data_payload, url_to_use, files_for_request, messages, identifier,
        ...
        retries = 0
        single_request_timeout = max(
            5, self.timeout / (self.max_retries + 1) if self.max_retries > 0 else self.timeout)
        ...
        try:
            ...
            while retries <= self.max_retries and (time.time() - start_time) < self.timeout:
                try:
                    ...
                    if files_for_request:
                        for field, (name, content_spec, mimetype) in files_for_request.items():
                            # Handles BytesIO and opened file objects
                            if isinstance(content_spec, io.IOBase):
                                content_spec.seek(0)
                                current_files_for_request[field] = (
                                    name, content_spec, mimetype)
                    ...
                    messages.append(
                        f"Data sent successfully to {url_to_use}: {response.status_code}")
                    if cache_entry and self.caching_enabled:
                        # _remove_from_cache_by_item will acquire the lock
                        self._remove_from_cache_by_item(cache_entry)
                    return messages
                except requests.exceptions.Timeout:
                    messages.append(
                        f"Request timeout (attempt {retries + 1}/{self.max_retries + 1}) to {url_to_use}")
                except requests.exceptions.RequestException as e:
                    messages.append(
                        f"Error sending data (attempt {retries + 1}/{self.max_retries + 1}) to {url_to_use}: {e}")
                    if hasattr(e, 'response') and e.response is not None:
                        messages.append(
                            f"Response status: {e.response.status_code}, Response text: {e.response.text[:200]}...")

                if self.debug and 'e' in locals() and hasattr(e, 'request'):
                    debug_details = [
                        f"  [DEBUG] Request: {e.request.method} {e.request.url}"]
                    if e.request.headers:
                        debug_details.append(
                            f"  [DEBUG] Request Headers: {e.request.headers}")
                    if e.request.body:
                        body_repr = repr(e.request.body)
                        debug_details.append(
                            f"  [DEBUG] Request Body: {body_repr[:200] + '...' if len(body_repr) > 200 else body_repr}")
                    if hasattr(e, 'response') and e.response is not None:
                        debug_details.append(
                            f"  [DEBUG] Response Status: {e.response.status_code}")
                        if e.response.headers:
                            debug_details.append(
                                f"  [DEBUG] Response Headers: {e.response.headers}")
                ...
                if retries <= self.max_retries and (time.time() - start_time) < self.timeout:
                    sleep_time = self.retry_delay * \
                        (2**(retries-1)) + \
                        random.uniform(0, 1)  # Adjusted exponent
            ...
            messages.append(
                f"Failed to send data for '{identifier}' after {retries} attempts or overall timeout.")
            if not cache_entry and not is_heartbeat_type and self.caching_enabled:
                if url_to_use and self.cache_files_dir is not None:
                    # _add_to_cache will acquire the lock
                    self._add_to_cache(
                        data_payload, url_to_use, files_for_request, identifier, is_heartbeat_type, messages)
                elif not url_to_use:
                    messages.append(
                        f"URL for '{identifier}' was invalid, not caching.")
                elif self.cache_files_dir is None:
                    messages.append(
                        f"Cache files directory not available for '{identifier}', not caching files.")
            ...
            if files_opened_for_this_send:
                if self.debug:
                    messages.append(
                        f"[DEBUG] Closing {len(files_opened_for_this_send)} file objects opened for this send.")
                for f_obj in files_opened_for_this_send:
                    try:
                        f_obj.close()
                    except Exception as e_close:
                        messages.append(
                            f"[WARNING] Error closing cached file object for '{identifier}': {e_close}")

    def _thread_done_callback(self, future, identifier, data_type_description):
        try:
            messages = future.result()
            is_successful = any(
                "Data sent successfully" in msg for msg in messages)
            prefix = "[SUCCESS]" if is_successful else "[FAILURE]"
            print(
                f"\n{prefix} {data_type_description}: {identifier} - {self.source}")
            for msg in messages:
                print(f"  - {msg}")
            print("-" * (len(identifier) +
                  len(data_type_description) + len(prefix) + 5))
        except Exception as e:
            print(
                f"\n[ERROR] Thread execution error for {data_type_description}: {identifier} - {self.source}")
    ...
    def send_data(self, data, heartbeat=False, files=None):
        ...
        future.add_done_callback(
            lambda f: self._thread_done_callback(f, identifier, data_type_desc))
    ...
    def _start_cache_management_timer(self):
        ...
        if self.debug:
            print(
                f"[DEBUG] Cache management timer scheduled to run in {self.cache_retry_interval}s.")
    ...
    def _periodic_cache_management_task(self):
        ...
        if self.shutting_down_event.is_set():
            if self.debug:
                print(
                    "[DEBUG] Shutdown signalled, skipping periodic cache management.")
            return

        if self.debug:
            print(
                "[DEBUG] Running periodic cache management (enforcing limits and retrying)...")
        ...
        if not self.shutting_down_event.is_set():
            ...
            if self.debug:
                print(
                    "[DEBUG] Shutdown signalled after periodic cache management, not rescheduling.")
    ...
    def _enforce_cache_limits(self, save_after_enforce=False):
        ...
        with self.cache_lock:  # Acquire lock
            ...
            if self.max_cache_age_seconds > 0:
                ...
                # Iterate over a copy
                original_cache = list(self.failed_sends_cache)
                new_cache = []
                ...
                for item in original_cache:
                    if current_time - item.get('timestamp_failed', current_time) > self.max_cache_age_seconds:
                        if self.debug:
                            print(
                                f"[INFO] Pruning aged cache item (older than {self.max_cache_age_seconds}s): {item['original_identifier']}")
                        self._remove_files_for_cached_item(
                            item)  # Does not lock
                ...
                if self.debug and changed_in_memory:  # items_removed_by_age:
                    print(
                        f"[DEBUG] Pruned {len(items_removed_by_age)} items by age.")

            if self.max_cache_items > 0 and len(self.failed_sends_cache) > self.max_cache_items:
                num_to_prune = len(self.failed_sends_cache) - \
                    self.max_cache_items
                if self.debug:
                    print(
                        f"[INFO] Cache size ({len(self.failed_sends_cache)}) exceeds limit ({self.max_cache_items}). Pruning {num_to_prune} oldest items.")
                ...
                changed_in_memory = True
                if self.debug:
                    print(f"[DEBUG] Pruned {num_to_prune} items by count.")

            if changed_in_memory and save_after_enforce:
                if not self._save_cache():  # Does not lock
                    if self.debug:
                        print(
                            f"[CRITICAL] Failed to save cache after enforcing limits. Disk cache may be inconsistent.")
    ...
    def _cleanup_orphaned_cached_files(self):
        ...
        if not self.cache_files_dir or not os.path.isdir(self.cache_files_dir):
            if self.debug:
                print(
                    "[DEBUG] Cache files directory not configured or does not exist, skipping orphan cleanup.")
            return

        if self.debug:
            print(
                f"[INFO] Starting cleanup of orphaned files in {self.cache_files_dir}...")
                ...
                if item.get("cached_files"):
                    for _, file_info in item["cached_files"].items():
                        if file_info.get("cached_filepath"):
                            valid_cached_files.add(os.path.normpath(
                                file_info["cached_filepath"]))

        cleaned_count = 0
        try:
            for filename in os.listdir(self.cache_files_dir):
                filepath = os.path.normpath(
                    os.path.join(self.cache_files_dir, filename))
                if os.path.isfile(filepath):
                    if filepath not in valid_cached_files:
                        try:
                            ...
                            if self.debug:
                                print(
                                    f"  - Removed orphaned cache file: {filepath}")
                        except OSError as e:
                            if self.debug:
                                print(
                                    f"  - [ERROR] Failed to remove orphaned cache file {filepath}: {e}")
        except OSError as e:
            if self.debug:
                print(
                    f"[ERROR] Could not list cache files directory {self.cache_files_dir} for cleanup: {e}")
            return

        if cleaned_count > 0 and (self.debug):
            print(
                f"[INFO] Orphan cleanup complete. Removed {cleaned_count} file(s).")
    ...
    def _retry_failed_sends(self):
        ...
        with self.cache_lock:  # Acquire lock for initial scan
            if not self.failed_sends_cache:
                if self.debug:
                    print("[DEBUG] Cache is empty, no items to retry.")
                return

            if self.debug:
                print(
                    f"[INFO] Checking cache for items to retry. Current cache size: {len(self.failed_sends_cache)}")

            for item in self.failed_sends_cache:
                if item.get('cache_retry_attempts', 0) < self.max_cache_retries:
                    # Appending a reference, not a deep copy
                    items_to_process_this_round.append(item)
                elif self.debug:
                    print(
                        f"[INFO] Max cache retries ({self.max_cache_retries}) already met for item: {item['original_identifier']}. It will be pruned by age/size limits if applicable.")
        ...
        # item_snapshot_ref is a reference to an item in cache
        for item_snapshot_ref in items_to_process_this_round:
            if self.shutting_down_event.is_set():
                if self.debug:
                    print("[DEBUG] Shutdown signalled, stopping cache retry cycle.")
                break

            if processed_count >= self.max_workers:
                if self.debug:
                    print(
                        f"[INFO] Pausing cache retry cycle after processing {processed_count} items to not overwhelm workers.")
                break
            ...
            with self.cache_lock:  # Acquire lock for updating/removing item
                ...
                try:
                    ...
                    if item_snapshot_ref not in self.failed_sends_cache:
                        if self.debug:
                            print(
                                f"[DEBUG] Item {item_snapshot_ref.get('original_identifier')} no longer in live cache, skipping retry.")
                        continue
                    ...
                    if current_attempts >= self.max_cache_retries:
                        ...
                        if self.debug:
                            print(
                                f"[INFO] Max cache retries ({self.max_cache_retries}) reached for item: {live_item['original_identifier']}. Removing from cache.")
                    ...
                    if not self._save_cache():  # Does not lock
                        if self.debug:
                            print(
                                f"[CRITICAL] Failed to save cache after updating retry count for {live_item['original_identifier']}. Proceeding with retry attempt.")
                    ...
                    if self.debug:
                        print(
                            f"[ERROR] Unexpected error during cache lock for retry prep of {item_snapshot_ref.get('original_identifier')}: {e_lock}")
                    continue
            ...
            if live_item_for_retry:
                ...
                data_type_desc = "Cached Heartbeat Retry" if live_item_for_retry[
                    'is_heartbeat_type'] else "Cached Data Retry"
                ...
                if live_item_for_retry.get("cached_files"):
                    ...
                    if self.debug:
                        print(
                            f"[INFO] Retrying {retry_identifier} with {len(live_item_for_retry['cached_files'])} file(s) from disk.")
                    ...
                    for field_name, file_info in live_item_for_retry["cached_files"].items():
                        try:
                            ...
                            messages.append(
                                f"[ERROR] Failed to open cached file {file_info['cached_filepath']} for retry: {e_open}")
                            if self.debug:
                                print(
                                    f"[ERROR] Retrying {retry_identifier}: Could not open {file_info['cached_filepath']}: {e_open}")
                            all_cached_files_opened_successfully = False
                    if not all_cached_files_opened_successfully:
                        messages.append(
                            f"[WARNING] Retrying {retry_identifier} but one or more cached files could not be opened.")
                else:
                    if self.debug:
                        print(
                            f"[INFO] Retrying from cache (no files associated): {retry_identifier}")
                ...
                future.add_done_callback(lambda f: self._thread_done_callback(
                    f, retry_identifier, data_type_desc))
                processed_count += 1
                time.sleep(0.1)

        if processed_count > 0 and (self.debug):
            print(
                f"[INFO] Cache retry cycle submitted {processed_count} items for processing.")
        elif not items_to_process_this_round and (self.debug) and self.failed_sends_cache:
            # This condition might be tricky if items were removed during processing
            if self.debug:
                print(
                    f"[DEBUG] No cache items initially eligible or remaining for retry at this time.")

    def send_heartbeat(self, sn, timestamp, status_log="Heartbeat received successfully."):
        if not self.heartbeat_url:
            if self.debug:
                print("[ERROR] Heartbeat URL not configured. Cannot send heartbeat.")
            return
        data = {
        ...
        if self.debug:
            ...
            if self.debug:
                print(f"[DEBUG] Heartbeat data: {data}")
        self.send_data(data, heartbeat=True)

    def shutdown(self, wait=True):
        if self.debug:
            print("[INFO] Shutting down DataUploader...")
        self.shutting_down_event.set()

        if self.cache_management_timer and self.caching_enabled:
            if self.debug:
                print("[INFO] Cancelling cache management timer...")
        ...
        if self.debug:
            print("[INFO] Shutting down DataUploader executor...")
        self.executor.shutdown(wait=wait)

        if self.debug:
            print("[INFO] Performing final cache save...")
        with self.cache_lock and self.caching_enabled:  # Acquire lock for final save
            self._save_cache()  # Does not lock

        if self.debug:
            print("[INFO] DataUploader shut down complete.")
